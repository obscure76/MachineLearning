...
// 
//	$Revision: 1.6 $ $Date: 2001/11/03 20:57:29 $
//	$Log: bp.cc,v $
...
// 4		number of training input-output 
...
void processConfig (char *filename) {
  ...
  for (int i=0; i<numInp; ++i) {
	...
	for (int j=0; j<inpSize-biasFlag; ++j) {
		fscanf(fp,"%lf",&(dataIn->mat[i][j]));
	}
	if (biasFlag) dataIn->mat[i][inpSize-biasFlag]=1.0;
	// output
	for (int j=0; j<outSize; ++j) {
		fscanf(fp,"%lf",&(dataOut->mat[i][j]));
	}
	fscanf(fp,"\n");
  }
...
int main (int argc, char *argv[]) {
  ...
  while (1) {
	...
	// 6. Print status
		...
		cout << " : hidden "; hidden->print(fp);
		cout << endl;
        
		//cout << "TARGET: "; target->print(); 
		//cout << endl;
		cout << "\t\tOUT " << t <<" " ; output->print(fp);
		cout << endl;
         }
    } // end of one epoch
    //-------------------------------------------------- end for
