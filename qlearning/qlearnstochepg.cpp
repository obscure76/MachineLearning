    ...
    for(int i=0;i<4;i++)
    {
        ...
        while(1)
        {
            ...
            if(QMatrix->matr[state][action] !=-99)
            {
                ...
                alphan = (1.0/(1+visits->matr[state][action]));
                ...
                if(QMatrix->matr[state][action] - update < 0.00001)
                    //nochange++;
                    update = update;
                else
                    nochange=0;
        ...
        for(int state = 0; state < STATEMAX; state++)
        {
            for(int action = 0; action < ACTIONMAX; action++)
            {
                if(visits->matr[state][action] > 0)
                    Expectr->matr[state][action] = \
                    Expectr->matr[state][action] / visits->matr[state][action];
            }
        }
